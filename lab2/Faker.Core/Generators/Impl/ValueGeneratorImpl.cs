...
public class ObjectGenerator : IValueGenerator
{
    ...
    private object GenerateUnsafe(Type typeToGenerate, GeneratorContext context)
    {
        ...
        var ctors = typeToGenerate.GetConstructors();
        ConstructorInfo temp;
        for (int i = 0; i < ctors.Length; i++)
        {
            for (int j = i + 1; j < ctors.Length; j++)
            {
                if (ctors[i].GetParameters().Length < ctors[j].GetParameters().Length) 
                {
                    temp = ctors[i];
                    ctors[i] = ctors[j];
                    ctors[j] = temp;
        ...
        foreach (ConstructorInfo ctor in ctors)
        {
            try
            {
                var initializedParams = ctor.GetParameters()
                    .Select(parameterInfo => context.Faker.Create(parameterInfo.ParameterType))
                    .ToArray();
                obj = Activator.CreateInstance(typeToGenerate, initializedParams);
                break;
            } catch (Exception e)
            {
                Console.WriteLine(e);
